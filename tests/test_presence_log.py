...
def test_set_display_name(session: Session):
    ...
    result = set_display_name(session, "AA:BB:CC:DD:EE:FF", "My Phone")
    assert result is not None
    assert result.display_name == "My Phone"


def test_set_display_name_not_found(session: Session):
    assert set_display_name(session, "00:00:00:00:00:00", "Ghost") is None
...
def test_log_presence_change(session: Session):
    ...
    log = log_presence_change(session, "AA:BB:CC:DD:EE:FF", PresenceEvent.arrive)
    assert log.id is not None
    assert log.mac_address == "AA:BB:CC:DD:EE:FF"
...
def test_get_presence_history(session: Session):
    ...
    log_presence_change(session, "AA:BB:CC:DD:EE:FF", PresenceEvent.arrive)
    log_presence_change(session, "AA:BB:CC:DD:EE:FF", PresenceEvent.depart)
...
def test_get_presence_history_filtered(session: Session):
    ...
    log_presence_change(session, "AA:BB:CC:DD:EE:01", PresenceEvent.arrive)
    log_presence_change(session, "AA:BB:CC:DD:EE:02", PresenceEvent.arrive)

    history = get_presence_history(session, mac_address="AA:BB:CC:DD:EE:01")
    assert len(history) == 1
    assert history[0].mac_address == "AA:BB:CC:DD:EE:01"
...
def test_detect_presence_changes_departure(session: Session):
    store_module._previously_present = {"AA:BB:CC:DD:EE:FF"}

    old_time = datetime.now(UTC).replace(tzinfo=None) - timedelta(hours=1)
    session.add(Device(mac_address="AA:BB:CC:DD:EE:FF", last_seen=old_time))
    session.commit()
